...
if($do == 'mod') {
    ...
    if(!submitcheck('modsubmit')) {
        $company = C::t("common_member_profile")->fetch($_G['uid']);
        if (empty($company['height']) && empty($company['weight'])) { // 超级管理员
            ...
            $uid_arr = DB::fetch_all('SELECT uid from zm_common_member_profile WHERE height = '."'$height'"
            );
            $uid_str = '';
            foreach ($uid_arr as $key => $value) {
                $uid_str .= ',' . $value['uid'];
            }
            $uid_str = substr($uid_str, 1);
            // 根据待审核用户id获取管理员所管理的所有用户
            $count = DB::fetch_first("SELECT count(uid) as '0' from zm_common_member_validate WHERE uid IN ($uid_str)");
            $sendemail = isset($_GET['sendemail']) ? $_GET['sendemail'] : 0;
            $checksendemail = $sendemail ? 'checked' : '';
            $start_limit = ($page - 1) * $_G['setting']['memberperpage'];
            $validatenum = $count[0];
        ...
        if($validatenum) {
            ...
            if (empty($company['height']) && empty($company['weight'])) {
                ...
                $allvalidate  =  DB::fetch_all('SELECT mvi.field, v.message, v.submittimes, v.submitdate, v.moddate, v.admin, v.remark, v.uid as vuid
			...
			WHERE v.uid IN'. "(" . $uid_str . ")" .' and v.status = 0 ORDER BY v.submitdate DESC '.DB::limit($start_limit, $start_limit), '', 'vuid');
                $uids = array_keys($allvalidate);
                $allmember = C::t('common_member')->fetch_all($uids, false, 0);
                $allmemberstatus = C::t('common_member_status')->fetch_all($uids, false, 0);
                $allmemberprofile = C::t('common_member_profile')->fetch_all($uids, false, 0);
            ...
            foreach($allvalidate as $uid => $member) {
                ...
                $members .= "<tr class=\"hover\" id=\"mod_uid_{$member[uid]}\"><td class=\"rowform\" style=\"width:80px;\"><ul class=\"nofloat\">
			 	 
			 
				
				</li>
				<li><input id=\"mod_uid_{$member[uid]}_2\" class=\"radio\" type=\"radio\" name=\"modtype[$member[uid]]\" value=\"validate\" onclick=\"set_bg('validate', $member[uid]);\"><label for=\"mod_uid_{$member[uid]}_2\">$lang[validate]</label></li>\n".
                    "<li>".($member['groupid'] == 8 ? "<input id=\"mod_uid_{$member[uid]}_3\" class=\"radio\" type=\"radio\" name=\"modtype[$member[uid]]\" value=\"delete\" onclick=\"set_bg('delete', $member[uid]);\"><label for=\"mod_uid_{$member[uid]}_3\">$lang[delete]</label>" : "<input disabled class=\"radio\" type=\"radio\" />$lang[delete]")."</li>
					
					
					<a href='javascript:;'  onclick=\"sendemail('$member[email]','$member[uid]','1','$member[username]','$member[remark]',this);alert('SUCCESS');\">发送审核通过邮件</a>
					<br>
						<a href='javascript:;'  onclick=\"sendemail('wangyongh@chinacoal.com','$member[uid]','1','$member[username]','$member[remark]',this);alert('SUCCESS');\">发送审核通过邮件(管理员）</a>
					<br>
					<a href='javascript:;'  onclick=\"sendemail('$member[email]','$member[uid]','2','$member[username]','$member[remark]',this);alert('SUCCESS');\">发送审核不邮件</a>
						<br>
						<a href='javascript:;'  onclick=\"sendemail('wangyongh@chinacoal.com','$member[uid]','2','$member[username]','$member[remark]',this);alert('SUCCESS');\">发送审核不邮件(管理员）</a>
					
					</li></ul></td><td style='width:30%'><b><a href=\"home.php?mod=space&uid=$member[uid]\" target=\"_blank\">$member[username]</a></b>\n".$_G['cache']['usergroups'][$member['groupid']]['grouptitle'].
                    ...
                    "$lang[moderate_members_mod_time]: $member[moddate]</td><td>
					<textarea rows=\"4\" onblur=\"changehtml(this.value);\" id=\"remark[$member[uid]]\" name=\"remark[$member[uid]]\" style=\"width: 95%; word-break: break-all\">$member[remark]</textarea></td></tr>\n";
            }
        }
...
}else if($do == 'sendemails')
{
    ...
    {
        $title="双创论坛注册信息表";
        $content="您已成功提交注册信息给论坛管理员，请下载并打印附件，经单位人事部门确认人员真实性后，将文件签字扫描发送至管理员邮箱'wangyongh@chinacoal.com'<a href='".$_G['siteurl']."home.php?mod=space&do=export&uids=".$uids."'>   点击下载注册信息表！！</a>";
    }else{

        $title="双创论坛注册信息修改通知";
        $content="".$admin." 用户，您好。您注册时提交的注册信息有误，请点击此链接进行修改。修改建议".$res."<a href='http://172.16.3.91:8080/f/showMember.html?registAccount=".$admin."'> 进入修改页面！</a>";
    }
    $succeed = sendmail($tomail,$title,$content);
    if($succeed=="1")
    {
        echo "success";
    }else{
